...
export default function AdminDashboard() {
    ...
    const uniqueCustomers = new Set([
        ...calls.map(c => c.customer_phone),
        ...orders.map(o => o.customer_phone)
    ].filter(p => p)).size;
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {(() => {
                                        ...
                                        calls.forEach(c => {
                                            ...
                                            if (!clientsMap.has(phone)) {
                                                clientsMap.set(phone, {
                                                    phone,
                                                    name: c.customer_name || 'Desconocido',
                                                    calls: 0,
                                                    orders: 0,
                                            ...
                                            if (callDate > client.lastInteraction) {
                                                client.lastInteraction = callDate;
                                                if (c.customer_name) client.name = c.customer_name;
                                            }
                                        });
                                        ...
                                        orders.forEach(o => {
                                            ...
                                            if (orderDate > client.lastInteraction) {
                                                client.lastInteraction = orderDate;
                                                if (o.customer_name) client.name = o.customer_name;
                                            }
                                        });
                                        ...
                                        return Array.from(clientsMap.values())
                                            ...
                                            .map((client) => {
                                                const isFrequent = client.orders > 3;
                                                return (
                                                    <div key={client.phone} className="group relative p-6 rounded-[24px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all duration-300">
                                                        <div className="flex items-start justify-between mb-4">
                                                            ...
                                                            <div className="flex flex-col items-end gap-2">
                                                                <span className="text-[10px] font-black uppercase tracking-widest bg-white/5 px-2 py-1 rounded-full text-gray-400">
                                                                    {client.calls} Llamadas
                                                                </span>
                                                                {client.orders > 0 && (
                                                                    <span className="text-[10px] font-black uppercase tracking-widest bg-blue-500/10 px-2 py-1 rounded-full text-blue-400">
                                                        ...
                                                        <h4 className="text-lg font-bold text-white mb-1 flex items-center gap-2">
                                                            {client.name}
                                                            {isFrequent && (
                                                                <span className="text-[9px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded-md border border-yellow-500/30">VIP</span>
                                                            )}
                                                        </h4>
                                                        <p className="text-sm text-gray-400 font-mono mb-4">{client.phone}</p>

                                                        {isFrequent && (
                                                            <div className="mb-4 text-[10px] font-bold text-green-400 bg-green-500/5 p-2 rounded-lg border border-green-500/10 flex items-center gap-2">
            ...
            <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-2xl border-t border-white/10 px-6 py-4 flex justify-between items-center z-50">
                {
                    [
                        ...
                        { id: 'calls', icon: Phone },
                        { id: 'unique_clients', icon: UserPlus },
